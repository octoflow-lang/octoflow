...
2. **GPU Native, CPU on demand.** The GPU is the computer — all compute runs on GPU by default. The CPU handles I/O boundaries (file, network, console) and truly sequential scalar logic. This is not "GPU when beneficial" — it's GPU always, CPU when necessary. The programmer never specifies where code runs; the default is GPU, and the compiler only falls back to CPU for operations that are inherently sequential or I/O-bound.
...
| **Annex E** | Implementation Roadmap — phased validation strategy, Phase 0-10 with tasks for Claude Code, validation gates, risk registry, repository structure | Draft v0.1 |
