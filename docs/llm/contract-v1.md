...
for i in range(0, 10)      // range() only in for-loop syntax; use range_array(s,e) for arrays
    print("{i}")
end
...
// fn(x) lambdas work ONLY as arguments to filter/map_each/reduce/sort_by
...
Operators: `+ - * / %` `== != < > <= >=` `&& || !`
Strings: `"hello {name}"` interpolates variables. NO escape sequences — `\n` is literal. Use `chr(10)` for newline, `chr(9)` for tab, `chr(34)` for quote.
...
http_post_auth(url,body,bearer)    // POST with Authorization: Bearer header
...
// Note: stream DSL (tap/emit) is compiler-internal — do not generate stream syntax
```
...
**15. String Building (no escapes — use chr)**
```
let nl = chr(10)
let tab = chr(9)
let header = "Name" + tab + "Score" + nl
let row = "Alice" + tab + "95" + nl
let report = header + row
print("{report}")
```
